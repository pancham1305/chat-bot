...
const Signup = () => {
  ...
  const handleSubmit = async(e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      toast.loading('Signing up...', {id:"1ogin"});
      await auth?.signup(name, email, password);
      toast.success('Signed up successfully', {id:"1ogin"});
      
    } catch (error) {
      console.log(error);
      toast.error('Signup failed', {id:"1ogin"});
}
  }
  useEffect(() => {
    if (auth?.user) {
      ...
      <Box display={"flex"} flex={{ xs: 1, md: 0.5 }} justifyContent={'center'} alignItems={"center"} padding={2} ml={'auto'} mt={16}>
        <form onSubmit={handleSubmit} style={{ margin: 'auto', padding: "30px", boxShadow: "10px 10px 20px #000", borderRadius: '10px', border: "none" }}>
          <Box sx={{ display: 'flex', flexDirection: "column", justifyContent: "center" }}>
            ...
            <Button type='submit' sx={{px:2, py:1, mt:2, width:"428px", borderRadius:2, bgcolor:"#00fffc", ":hover":{bgcolor:"white", color:"black"}}} endIcon={<IoIosLogIn />}>Signup </Button>
          </Box>
        </form>
      </Box>
   </Box>
